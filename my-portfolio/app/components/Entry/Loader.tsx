...
const Loader = ({
    ...
    return (
        <div
            className={`fixed inset-0 z-[100] flex flex-col overflow-hidden transition-all duration-1000 ${isLoaded
                ? "opacity-0 pointer-events-none scale-110"
                : "opacity-100 pointer-events-auto scale-100"
                }`}
        >
            {/* GLASS OVERLAY (instead of black bg) */}
            <div className="absolute inset-0 backdrop-blur-xl bg-black z-0 " />
            ...
            {/* PROGRESS BAR */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 w-[60%] h-[2px] bg-white/10 rounded-full overflow-hidden z-10 backdrop-blur-md">
                <div
            ...
            >
                <span className="absolute top-[55%] left-1/2 -translate-x-1/2 text-white/80 text-sm tracking-widest z-10">
                    {loadingProgress}%
                </span>
            </div>
        </div>
